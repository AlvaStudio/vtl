...
uint256 hashGenesisBlock("0x142c9c6954ff721f1848f04685f509565d999b9f1ef4ea99dfe891fa8cadd72a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Vitalium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Vitalium Signed Message:\n";
...
{
    // Vitalium: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Vitalium: 2 days
static const int64 nTargetSpacing = 2.5 * 60; // Vitalium: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Vitalium: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x299c5fb49a9d859c19784839036e56bf3cf715b192c62534e00b328c14d81a6c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04467ed86ed7f02a58cf03f2c719a67835a6c602cb2c3dcfb76fd8fde2795da3037ad071759450e21c2fde5a3673b4457d5d9d0a8df9cae270f96fe44d07a51037") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528294071;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1920528;

        if (fTestNet)
        {
            block.nTime    = 1528294057;
            block.nNonce   = 275267;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xb8282c827d539355fea338e6608ec929124c26588ec0bd2b0b8f17707ac769c9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xa1, 0xb1, 0xd1 }; // Vitalium: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VitaliumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VitaliumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VitaliumMiner(CWallet *pwallet)
{
    printf("VitaliumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vitalium-miner");
    ...
    try { loop {
        ...
        printf("Running VitaliumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VitaliumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VitaliumMiner, pwallet));
}
